...
![](1n5d9p5_mpox-reuters_625x300_24_August_24.webp)

This is an analysis carried out on an M-Pox Data-set from \<htpps://catalog.ourworldindata.org\>:
...
Date (character) variable is converted to a date data type,
...
Death rate for each observation is calculated Formula= Total Death / Total Cases x 100
...
head(MPox_Death_Rate)
```

### Insightful Data Grouping

Data that provides key information such as: Average Case reported, Death Rate, New cases, Suspected cases in each region are calculated and grouped into a table

```{r}
# Average Total Cases
Afri_TC <- mean(Afri$total_cases)
Asia_TC <- mean(Asia$total_cases)
Euro_TC <- mean(Euro$total_cases)
Isl_TC <- mean(Isl$total_cases)
Nt_Ame_TC <- mean(Nt_Ame$total_cases)
Ocea_TC <- mean(Ocea$total_cases)
St_Ame_TC <- mean(St_Ame$total_cases)
Un_Spe_TC <- mean(Un_Spe$total_cases)
...
```{r}
# Average Total Death
Afri_td <- mean(Afri$total_deaths )
Asia_td <- mean(Asia$total_deaths )
Euro_td <- mean(Euro$total_deaths )
Isl_td <- mean(Isl$total_deaths )
Nt_Ame_td <- mean(Nt_Ame$total_deaths )
Ocea_td <- mean(Ocea$total_deaths )
St_Ame_td <- mean(St_Ame$total_deaths )
Un_Spe_td <- mean(Un_Spe$total_deaths )
...
Afri_sc <- mean(Afri$suspected_cases_cumulative )
Asia_sc <- mean(Asia$suspected_cases_cumulative )
Euro_sc <- mean(Euro$suspected_cases_cumulative )
Isl_sc <- mean(Isl$suspected_cases_cumulative )
Nt_Ame_sc <- mean(Nt_Ame$suspected_cases_cumulative )
Ocea_sc <- mean(Ocea$suspected_cases_cumulative )
St_Ame_sc <- mean(St_Ame$suspected_cases_cumulative )
Un_Spe_sc <- mean(Un_Spe$suspected_cases_cumulative )
...
Graphical representation of Data visual analysis
...
<https://github.com/Astro0re/Data/tree/main/Monkey%20_Pox>
